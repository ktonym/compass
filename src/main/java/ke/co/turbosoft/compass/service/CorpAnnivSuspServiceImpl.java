...
public class CorpAnnivSuspServiceImpl extends  AbstractService implements CorpAnnivSuspService {
    ...
    @Override
    @Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
    public Result<List<CorpAnnivSuspension>> findAllInAnniv(Integer idCorpAnniv, String actionUsername) {
        if( !isValidUser(actionUsername) ){
            return ResultFactory.getFailResult(USER_INVALID);
        }
    ...
    @Override
    @Transactional(readOnly = true, propagation = Propagation.SUPPORTS)
    public Result<List<CorpAnnivSuspension>> findAll(Integer idCorporate,
                                               String actionUsername) {
        if( !isValidUser(actionUsername) ){
            return ResultFactory.getFailResult(USER_INVALID);
        }
